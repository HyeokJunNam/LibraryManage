import { createBorrowBookProcess } from "./borrow-panel.js";
import { createReturnBookProcess } from "./return-panel.js";
import { createBookSearchModal } from "../modal/book-search-modal.js";

export function createBookProcess({
                                      getSelectedMemberId
                                  } = {}) {
    const circulationEmptyPanel = document.getElementById("circulationEmptyPanel");
    const circulationEmptyPanelContent = document.getElementById("circulationEmptyPanelContent");

    const idlePanelPrototype = circulationEmptyPanelContent?.firstElementChild?.cloneNode(true) ?? null;
    ...
    function setBookPanelMode(mode) {
        currentMode = mode;
        circulationEmptyPanel?.setAttribute("data-mode", mode);
    }
    ...
    function mountPanel(panelElement) {
        if (!circulationEmptyPanelContent || !panelElement) {
            return;
        }

        circulationEmptyPanelContent.replaceChildren(panelElement);
    }
